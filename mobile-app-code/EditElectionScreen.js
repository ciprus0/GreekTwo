...
export default function EditElectionScreen({ navigation, route }) {
  ...
  const loadElectionData = () => {
    ...
    if (election.start_time) {
      const startDateTime = new Date(election.start_time);
      setStartDate(localDateString(startDateTime));
      setStartTime(`${pad(startDateTime.getHours())}:${pad(startDateTime.getMinutes())}`);
      setSelectedTimeValue(startDateTime);
    }
    
    // Parse end time
    if (election.end_time) {
      const endDateTime = new Date(election.end_time);
      setEndDate(localDateString(endDateTime));
      setEndTime(`${pad(endDateTime.getHours())}:${pad(endDateTime.getMinutes())}`);
      setSelectedEndTimeValue(endDateTime);
    }
  ...
  const handleSubmit = async () => {
    ...
    if (positions.some(pos => pos.nominees.length === 0 || pos.nominees.some(nom => !nom.trim()))) {
      Alert.alert('Error', 'Please enter nominees for all positions');
      return;
    }
    ...
    try {
      ...
      // Format positions data for JSONB column
      const positionsData = positions.map(position => ({
        title: position.title.trim(),
        nominees: position.nominees.filter(nom => nom.trim())
      }));
  ...
  const renderHeader = () => (
    <View style={[styles.header, { backgroundColor: colors.surface }]}>
      ...
      <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Election</Text>
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.section}>
        ...
        <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>
          Update the basic details for your election
        </Text>
  ...
  const renderDateTime = () => (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.section}>
        ...
        <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>
          Update when the election will start and end
        </Text>
  ...
  const renderPositions = () => (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>Positions & Nominees</Text>
        <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>
          Update the positions and their nominees
        </Text>
        ...
        <View style={styles.positionsContainer}>
          {positions.map((position, positionIndex) => (
            <View key={positionIndex} style={[styles.positionCard, { backgroundColor: colors.surface }]}>
              ...
              <View style={styles.inputGroup}>
                <Text style={[styles.inputLabel, { color: colors.text }]}>Nominees *</Text>
                {position.nominees.map((nominee, nomineeIndex) => (
                  <View key={nomineeIndex} style={styles.nomineeRow}>
                    <TextInput
                      style={[styles.nomineeInput, { 
                        backgroundColor: colors.background, 
                        color: colors.text,
                        borderColor: colors.border 
                      }]}
                      value={nominee}
                      onChangeText={(text) => updateNominee(positionIndex, nomineeIndex, text)}
                      placeholder={`Nominee ${nomineeIndex + 1}`}
                      placeholderTextColor={colors.textSecondary}
                      returnKeyType="next"
                      blurOnSubmit={false}
                    />
                    {position.nominees.length > 1 && (
                      <TouchableOpacity onPress={() => removeNominee(positionIndex, nomineeIndex)}>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => addNominee(positionIndex)}
                >
                  <Feather name="plus" size={16} color={colors.primary} />
                  <Text style={[styles.addButtonText, { color: colors.primary }]}>Add Nominee</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))}
