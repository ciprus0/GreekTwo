...
export interface Announcement {
  ...
  content: string
  category: string
  channel?: string // Add channel support
  author_id: string
  author_name: string
...
export const api = {
  ...
  async getAnnouncementChannels(organizationId: string): Promise<AnnouncementChannel[]> {
    if (!organizationId) return []

    const cacheKey = getCacheKey("announcement_channels", organizationId)
    ...
    return withCache(
      ...
      async () => {
        const { data: org } = await supabase
          .from("organizations")
          .select("announce_channels")
          .eq("id", organizationId)
          .single()

        if (!org || !org.announce_channels) {
          // Return default channels if none are configured
          return [
            { id: "general", name: "General", color: "#4F8EF7", description: "General announcements" },
            { id: "events", name: "Events", color: "#A259F7", description: "Event announcements" },
            { id: "important", name: "Important", color: "#F75C4E", description: "Important updates" },
            { id: "recruitment", name: "Recruitment", color: "#4F8EF7", description: "Recruitment announcements" }
          ]
        }

        return org.announce_channels as AnnouncementChannel[]
      },
  ...
  async createAnnouncementChannel(
    organizationId: string, 
    channelData: Omit<AnnouncementChannel, "id">
  ): Promise<AnnouncementChannel> {
    if (!organizationId) throw new Error("Organization ID is required")

    // Get current channels
    const currentChannels = await this.getAnnouncementChannels(organizationId)
    
    // Generate unique ID
    const newChannel: AnnouncementChannel = {
      ...channelData,
      id: channelData.name.toLowerCase().replace(/\s+/g, "-")
    }

    // Check if ID already exists
    if (currentChannels.find(ch => ch.id === newChannel.id)) {
    ...
    const { error } = await supabase
      .from("organizations")
      .update({ announce_channels: updatedChannels })
      .eq("id", organizationId)

    if (error) {
      throw new Error(`Failed to create announcement channel: ${error.message}`)
    }

    // Clear cache
    cacheManager.delete(getCacheKey("announcement_channels", organizationId))

    return newChannel
  },

  async updateAnnouncementChannel(
    organizationId: string,
    channelId: string,
    updates: Partial<AnnouncementChannel>
  ): Promise<AnnouncementChannel | null> {
    ...
    const updatedChannel = { ...currentChannels[channelIndex], ...updates }
    currentChannels[channelIndex] = updatedChannel

    // Update organization
    const { error } = await supabase
      .from("organizations")
      .update({ announce_channels: currentChannels })
      .eq("id", organizationId)

    if (error) {
      throw new Error(`Failed to update announcement channel: ${error.message}`)
    }
  ...
  async deleteAnnouncementChannel(organizationId: string, channelId: string): Promise<boolean> {
    if (!organizationId || !channelId) return false
    ...
    if (error) {
      throw new Error(`Failed to delete announcement channel: ${error.message}`)
    }
